...
import { MobileNewsCardExpanded } from './MobileNewsCardExpanded'

interface Props {
  card: NewsCard
}

export function MobileNewsCard({ card }: Props) {
  ...
  const toggle = () => {
    if (isExpanded) {
      ...
      const target = contentHeight > 0 ? contentHeight : 400
